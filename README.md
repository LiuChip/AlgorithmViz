...
| **CMake** | 构建系统 |
| **QGraphicsView / QGraphicsScene / QGraphicsObject** | 画布与图形项管理 |
| **QPainter** | 自定义形状绘制 |
...
│       └── anchor_resolver.*    # 锚点吸附解析器（已完成）
├── shapes/
│   ├── shape.h / .cpp          # 形状基类（抽象类，继承 QGraphicsObject）
...
│   └── connector/
│       ├── connector_anchor.h / .cpp # 连接线端点绑定数据结构
│       └── connector.h / .cpp  # 连接线图形类（已完成）
├── editor/
...
tests/
└── connector_test.cpp          # 连接器系统自动化回归测试（36 个用例）
```

## ✅ 已完成模块

### Shape 基类（完整）
- 继承 `QGraphicsObject`，支持 Qt 信号槽，纯虚 `boundingRect()` + `paint()`
- 三个结构体：`Border`、`FillStyle`、`TextStyle`（各带无参/带参构造）
- 属性：位置(x,y,width,height)、旋转、边框/填充/文本样式、ID、name、visible、layer、lock_stat
- 几何修改方法全部返回 `bool` 并内置 `std::isfinite` / 非负值校验
- `itemChange()` 在未锁定状态下也会拦截 NaN / Inf 几何值
...
### Connector 连接线系统（已完成）
- `Connector` 直接继承 `Shape`（不继承 `ConnectableShape`），从结构上杜绝连线自连接循环
- `ConnectorAnchor` 端点数据结构：支持 Free / Boundary / Interior 三种绑定模式
- 目标图形删除后自动安全降级至 Free 模式（基于 QPointer + fallback 安全气囊）
- `AnchorResolver` 吸附解析器：Scene 物理坐标系精确距离计算、二道吸附校验防线
...
| **ConnectorController** | 连线交互控制器（鼠标驱动的连线创建与吸附预览） |
...
- ✅ Connector 连接线 + ConnectorAnchor + AnchorResolver — 已完成
- 🔄 交互与控制（ConnectorController / CanvasController）— 待实现
- ❌ 导出与编辑器 — 待实现
