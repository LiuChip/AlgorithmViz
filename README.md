...
│   ├── text_label.h / .cpp     # 文本标签（自动计算尺寸）
│   ├── line_shape.h / .cpp     # 线段
│   ├── arrow_shape.h / .cpp    # 单箭头（继承 LineShape）
│   └── dual_arrow_shape.h / .cpp # 双箭头（继承 ArrowShape）
├── editor/
...
- 属性：位置(x,y,width,height)、旋转、边框/填充/文本样式、ID、name、visible、layer、lock_stat、anchorPoints
- 静态自增 ID 和 Layer 计数器
...
| `TextLabel` | 纯文字标签，自动计算尺寸（QFontMetricsF） |
| `LineShape` | 线段，起点终点 + 外切矩形管理 |
...
| **ArrowShape paint 坐标修复** | LineShape 的 start/end 是场景坐标，需转本地坐标 |
