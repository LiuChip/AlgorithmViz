...
LineShape::LineShape(QPointF start, QPointF end)
    : Shape(QRectF(start, end).normalized().topLeft(), QRectF(start, end).normalized().size()),
      startPoint(start),
      endPoint(end)
{
}
...
QPointF LineShape::getStartPoint() const
{
    return startPoint;
}
...
QPointF LineShape::getEndPoint() const
{
    return endPoint;
}
...
void LineShape::setEndPoint(qreal x, qreal y)
{
